...
// UPDATED STRUCTURE: Strict Mapping (Project -> Tower -> Floors)
let structuralHierarchy = JSON.parse(localStorage.getItem("qa_strict_hierarchy")) || {
    "Fragrance": {
        "Tower-A": ["GF", "1st Floor", "2nd Floor"],
        "Tower-B": ["GF", "1st Floor"]
    },
    "Eutopia": {
        "B1": ["Basement", "GF"],
        "STP": ["Area-1"]
...
window.addEventListener('online', () => {
    document.getElementById('networkStatus').className = "network-badge online";
    document.getElementById('networkStatus').innerHTML = '<i class="fas fa-wifi"></i> Online';
    syncOfflineData();
});
window.addEventListener('offline', () => {
    document.getElementById('networkStatus').className = "network-badge offline";
    document.getElementById('networkStatus').innerHTML = '<i class="fas fa-wifi-slash"></i> Offline';
});
...
function processLogin() {
    ...
    if(validUser) {
        ...
        err.style.display = "block"; err.innerText = "Invalid credentials.";
    }
}
...
function activateApp() {
    ...
    if(currentUser.role === "user" && currentUser.permission === "view") {
        document.getElementById("navEntryBtn").style.display = "none"; showSection('dashboard');
    } else { showSection('entry'); }

    refreshDropdowns();
    initCanvas('entry'); initCanvas('modal');
    loadDefectsFromCloud(false);
    startAutoRefresh(); // Start background sync
...
function getAllowedProjects() {
    ...
    return currentUser.projects.filter(p => structuralHierarchy[p]);
}
...
function populateTowers() {
    ...
    if(p && structuralHierarchy[p]) {
        Object.keys(structuralHierarchy[p]).forEach(t => tSel.appendChild(new Option(t, t)));
    }
    populateFloors();
}
...
function populateFloors() {
    ...
    if(p && t && structuralHierarchy[p][t]) {
        structuralHierarchy[p][t].forEach(f => fSel.appendChild(new Option(f, f)));
    }
    document.getElementById("entryMapWarning").style.display = "none";
    canvasConfig.entry.marker = null; drawCanvas('entry');
...
function loadEntryMap() {
    ...
    if(base64Img) {
        ...
        const img = new Image();
        img.onload = () => { canvasConfig.entry.img = img; const canvas = document.getElementById('entryCanvas'); canvas.width = img.width; canvas.height = img.height; drawCanvas('entry'); };
        img.src = base64Img;
    } else {
...
function drawCanvas(type) {
    ...
    if(c.marker) {
        ...
        c.ctx.fillStyle = "#ef4444"; c.ctx.fill(); c.ctx.lineWidth = 4; c.ctx.strokeStyle = "#ffffff"; c.ctx.stroke();
    }
}
...
async function saveDefect(){
    ...
    const dueStr = document.getElementById("dueDate").value || null;
    let delay = "On Time";
    ...
    if(!navigator.onLine) {
        ...
        document.getElementById("defectForm").reset(); clearTempPhotos(); canvasConfig.entry.marker = null; drawCanvas('entry');
        return;
    }
    ...
    try {
        ...
        if(res.ok) {
            alert("Record Logged Successfully!"); document.getElementById("defectForm").reset(); clearTempPhotos(); canvasConfig.entry.marker = null; drawCanvas('entry');
...
async function syncOfflineData() {
    ...
    localStorage.removeItem('qa_offline_queue');
    if(successCount > 0) { alert(`Synced ${successCount} offline records successfully!`); loadDefectsFromCloud(false); }
}

// AUTO REFRESH LOGIC
function startAutoRefresh() {
    autoSyncInterval = setInterval(() => { if(navigator.onLine) loadDefectsFromCloud(true); }, 20000); // 20 Seconds
}
...
async function submitEditDefect() {
    ...
    if(stat === "Closed" && editTempPhotos.length === 0) return alert("Must add Final Photo to close defect.");
    ...
    try {
        ...
        if(res.ok) { alert("Updated Successfully!"); closeEditModal(); await loadDefectsFromCloud(false); } 
        else throw await res.json();
...
function openDrillModal(title, data) {
    ...
    document.getElementById("modalTitle").innerHTML = `<i class="fas fa-search-plus text-cyan"></i> Drill-Down: ${title} (${data.length})`;
...
// ====== BI TELEMETRY ADVANCED ANALYTICS ======
let chartsObj = {};
...
function renderCharts() {
    ...
    // Secondary Analytic mapping
    let anaMap = {}; let anaLabel = ""; let anaKey = "";
    if(filterAnalytic === "intensity") { anaKey = "intensity"; anaLabel = "Risk Spectrum"; }
    else if(filterAnalytic === "tower") { anaKey = "tower"; anaLabel = "Tower Segments"; }
    else if(filterAnalytic === "defect") { anaKey = "Type"; anaLabel = "Category Spread"; }
    else if(filterAnalytic === "floor") { anaKey = "floor"; anaLabel = "Floor Mapping"; }
...
// ====== SYSTEM SETUP (WITH EDIT/DELETE logic) ======
function renderAdminTables() {
    ...
    if(hBody) {
        ...
        Object.keys(structuralHierarchy).forEach(p => {
            Object.keys(structuralHierarchy[p]).forEach(t => {
                const floors = structuralHierarchy[p][t].join(", ");
                hHtml += `<tr><td><b>${p}</b></td><td>${t}</td><td style="white-space:normal; max-width:200px;">${floors}</td><td>
                    <button class="action-icon-btn edit-btn" onclick="editHierarchy('${p}','${t}')">Edit</button>
                    <button class="action-icon-btn del-btn" onclick="delHierarchy('${p}','${t}')">Del</button>
                </td></tr>`;
            });
        });
    ...
    if(cBody) cBody.innerHTML = Object.keys(defectMatrix).map(c => `<tr><td><b>${c}</b></td><td style="white-space:normal; max-width:200px;">${defectMatrix[c].join(", ")}</td><td>
        <button class="action-icon-btn edit-btn" onclick="editCategory('${c}')">Edit</button>
        <button class="action-icon-btn del-btn" onclick="delCategory('${c}')">Del</button>
    </td></tr>`).join('');
    ...
    if(fBody) {
        fBody.innerHTML = Object.keys(floorMaps).map(k => {
            const parts = k.split('_'); return `<tr><td>${parts[0]}</td><td>${parts[1]}</td><td>${parts[2]}</td>
            <td><img src="${floorMaps[k]}" width="40" height="40" style="object-fit:cover; border-radius:4px; cursor:pointer;" onclick="openZoomImage('${floorMaps[k]}')"></td>
            <td><button class="action-icon-btn del-btn" onclick="delMap('${k}')">Del</button></td></tr>`;
        }).join('');
    }
}
...
function saveHierarchy() {
    ...
    const f = document.getElementById("setupFloors").value.split(",").map(s=>s.trim()).filter(Boolean); 
    const editKey = document.getElementById("editHierarchyKey").value;

    if(editKey) {
        const [oldP, oldT] = editKey.split("|||");
        if(oldP !== p || oldT !== t) delete structuralHierarchy[oldP][oldT];
    }

    if(!structuralHierarchy[p]) structuralHierarchy[p] = {};
    structuralHierarchy[p][t] = f;
...
function editHierarchy(p, t) {
    document.getElementById("setupProjName").value = p; document.getElementById("setupTowerName").value = t;
    document.getElementById("setupFloors").value = structuralHierarchy[p][t].join(", ");
    document.getElementById("editHierarchyKey").value = `${p}|||${t}`;
    document.getElementById("btnSaveHierarchy").innerHTML = "<i class='fas fa-save'></i> Update";
}
function delHierarchy(p, t) {
    if(confirm(`Delete ${t} from ${p}?`)) {
        delete structuralHierarchy[p][t];
        if(Object.keys(structuralHierarchy[p]).length === 0) delete structuralHierarchy[p];
        localStorage.setItem("qa_strict_hierarchy", JSON.stringify(structuralHierarchy)); refreshDropdowns(); renderAdminTables();
    }
}
...
function saveCategory() {
    ...
    const s = document.getElementById("setupSpecs").value.split(",").map(x=>x.trim()).filter(Boolean);
    const editKey = document.getElementById("editCategoryKey").value;
    
    if(editKey && editKey !== c) delete defectMatrix[editKey];
    defectMatrix[c] = s; 
...
function editCategory(c) {
    document.getElementById("setupCatName").value = c; document.getElementById("setupSpecs").value = defectMatrix[c].join(", ");
    document.getElementById("editCategoryKey").value = c; document.getElementById("btnSaveCategory").innerHTML = "<i class='fas fa-save'></i> Update";
}
...
function populateMapSetupTowers() {
    ...
    const tSel = document.getElementById("mapSetupTower"); tSel.innerHTML = '<option value="">Tower</option>';
...
function populateMapSetupFloors() {
    const p = document.getElementById("mapSetupProject").value; const t = document.getElementById("mapSetupTower").value;
    const fSel = document.getElementById("mapSetupFloor"); fSel.innerHTML = '<option value="">Floor</option>';
...
function saveMapDrawing(e) {
    ...
    reader.onload = ev => {
        const img = new Image(); img.onload = () => {
            ...
            localStorage.setItem("qa_floorMaps", JSON.stringify(floorMaps)); alert("Floor Map Successfully Saved!"); renderAdminTables();
        }; img.src = ev.target.result;
...
function saveSystemUser() {
    ...
    let selProjects = [];
    if(role === "admin") selProjects = ["All"];
    else { document.querySelectorAll(".proj-chk:checked").forEach(cb => selProjects.push(cb.value)); if(selProjects.length === 0) return alert("Select at least one project."); }
    ...
    localStorage.setItem("qa_users", JSON.stringify(USER_MATRIX)); alert("User Access Saved!"); document.getElementById("suEmail").value = ""; document.getElementById("suPass").value = ""; renderUserTable();
}
function renderUserTable() {
    ...
    tbody.innerHTML = USER_MATRIX.map(u => {
        ...
        return `<tr><td><b>${u.id}</b></td><td>${u.role.toUpperCase()}</td><td style="white-space:normal; max-width:150px;">${pBadge}</td><td>${u.permission === "edit" ? "Full" : "View"}</td>
        <td>${u.id === currentUser.id ? "<i>(You)</i>" : `<button class="action-icon-btn del-btn" onclick="deleteUser('${u.id}')">Del</button>`}</td></tr>`;
    }).join('');
}
...
// PDF & Excel Export Methods (Unchanged, retaining functionality)
async function exportExcelWithPhotos(dataToExport) { /* same logic... */ 
...
function exportPDF(dataToExport){ 
    ...
    dataToExport.forEach((d)=>{
        html += `<div class="card"><div class="grid"><div><b>Project:</b> ${d.project} | <b>Tower:</b> ${d.tower} | <b>Floor:</b> ${d.floor}<br/><b>Category:</b> ${d.Type}</div>
        <div><b>Status:</b> ${d.status}<br/><b>Dates:</b> ${d.loggedDate}</div></div>
        <div style="margin-top:10px;"><b>Initial: </b>${d.initialPics.map(src=> `<img src="${src}" />`).join("")}</div></div>`;
    });
